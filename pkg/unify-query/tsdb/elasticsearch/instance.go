...
func (i *Instance) getAlias(ctx context.Context, db string, needAddTime bool, start, end time.Time, timezone, sourceType string) ([]string, error) {
	var (
		aliases []string
	...
	// bkdata 数据源使用东八区创建别名，所以需要特殊处理该逻辑
...
func (i *Instance) QueryRawData(ctx context.Context, query *metadata.Query, start, end time.Time, dataCh chan<- map[string]any) (int64, metadata.ResultTableOptions, error) {
	...
	aliases, err := i.getAlias(ctx, query.DB, query.NeedAddTime, start, end, query.Timezone, query.SourceType)
	if err != nil {
		return total, resultTableOptions, err
	}
...
func (i *Instance) QuerySeriesSet(
	...
	go func() {
		...
		aliases, err1 := i.getAlias(ctx, query.DB, query.NeedAddTime, start, end, query.Timezone, query.SourceType)
		if err1 != nil {
