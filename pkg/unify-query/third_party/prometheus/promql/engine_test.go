...
func TestPointSliceAllocationFollowsSparseOutput(t *testing.T) {
	pointPool = sync.Pool{}

	points := getPointSlice(44_400)
	require.LessOrEqual(t, cap(points), maxPointsSliceSize)

	points = getPointSlice(initialPointSliceSize(44_400))
	require.LessOrEqual(t, cap(points), initialPointsSliceSize)
}

func TestPointSlicePoolDropsDenseRangeCapacity(t *testing.T) {
	pointPool = sync.Pool{}
	putPointSlice(make([]Point, 0, maxPointsSliceSize+1))

	points := getPointSlice(initialPointsSliceSize)
	require.Equal(t, initialPointsSliceSize, cap(points))
}

func TestDenseAndLightPointSlicesAlternateWithoutPoolPollution(t *testing.T) {
	pointPool = sync.Pool{}
	for i := 0; i < 20; i++ {
		putPointSlice(make([]Point, 0, maxPointsSliceSize+1))
		light := getPointSlice(1)
		require.LessOrEqual(t, cap(light), 2)
		putPointSlice(light)
	}
}

func TestSparseRangeEvaluationHonorsCancellation(t *testing.T) {
	var input strings.Builder
	input.WriteString("load 1m\n")
	for i := 0; i < 1000; i++ {
		fmt.Fprintf(&input, "cancel_metric{series=%q} %d\n", fmt.Sprint(i), i)
	}
	...
	query, err := engine.NewRangeQuery(
		test.Queryable(),
		nil,
		"count_over_time(cancel_metric[1d])",
		start,
		start.Add(30*24*time.Hour-time.Minute),
		time.Minute,
	)
	require.NoError(t, err)
	defer query.Close()
	...
	require.ErrorAs(t, result.Err, &canceled)
}

func TestRangeFunctionEarlyMaxSamplesCheckPreservesV042Boundary(t *testing.T) {
	test, err := NewTest(t, `
load 1s
boundary_metric 0 1 2 3 4 5 6 7
`)
	require.NoError(t, err)
	require.NoError(t, test.Run())
	defer test.Close()
	...
	run := func(maxSamples int) *Result {
		t.Helper()
		engine := NewEngine(EngineOpts{
			MaxSamples: maxSamples,
			Timeout:    time.Minute,
		})
		query, queryErr := engine.NewRangeQuery(
			test.Queryable(),
			nil,
			"count_over_time(boundary_metric[8s])",
			start,
			end,
			time.Second,
		)
	...
	require.NoError(t, err)
	require.Equal(t, 16, matrix.TotalSamples())

	result = run(16)
	var tooManySamples ErrTooManySamples
	require.ErrorAs(t, result.Err, &tooManySamples)
}

func TestReusePointSliceDoesNotAliasPreviousSeriesCapacity(t *testing.T) {
	previous := &Series{Points: make([]Point, 2, 8)}
	previous.Points[0].T = 1
	previous.Points[1].T = 2

	next := reuseOrGetPointSlice(previous, 44_400)
	require.Equal(t, 2, cap(previous.Points))
	require.Equal(t, 6, cap(next))

	next = append(next, Point{T: 3})
	require.Equal(t, int64(1), previous.Points[0].T)
	require.Equal(t, int64(2), previous.Points[1].T)
}

func BenchmarkSparseRangePointCapacity(b *testing.B) {
	var input strings.Builder
	input.WriteString("load 1m\n")
	for i := 0; i < 1000; i++ {
		fmt.Fprintf(&input, "sparse_metric{series=%q} %d\n", fmt.Sprint(i), i)
	}
	test, err := NewTest(b, input.String())
	require.NoError(b, err)
	require.NoError(b, test.Run())
	b.Cleanup(test.Close)
	...
	start := time.Unix(0, 0)
	end := start.Add(30*24*time.Hour - time.Minute)
	...
	for i := 0; i < b.N; i++ {
		...
		beforeRSS := currentRSSBytes()

		query, queryErr := engine.NewRangeQuery(
			test.Queryable(),
			nil,
			"count_over_time(sparse_metric[1d])",
			start,
			end,
			time.Minute,
		)
		require.NoError(b, queryErr)
		result := query.Exec(test.Context())
		require.NoError(b, result.Err)
		matrix, matrixErr := result.Matrix()
		require.NoError(b, matrixErr)

		var effectivePoints, pointCapacity int
		for _, series := range matrix {
			effectivePoints += len(series.Points)
			pointCapacity += cap(series.Points)
		}
		capacityBytes := pointCapacity * int(unsafe.Sizeof(Point{}))
		var after runtime.MemStats
		runtime.ReadMemStats(&after)
		afterRSS := currentRSSBytes()
		b.ReportMetric(float64(effectivePoints), "effective-points")
		b.ReportMetric(float64(capacityBytes), "point-capacity-bytes")
		b.ReportMetric(float64(after.HeapAlloc-before.HeapAlloc), "heap-delta-bytes")
		if afterRSS >= beforeRSS {
			b.ReportMetric(float64(afterRSS-beforeRSS), "rss-delta-bytes")
		}
		require.Less(b, capacityBytes, 200*1024*1024)
		query.Close()
	}
}

func BenchmarkDenseRangePointCapacity(b *testing.B) {
	var input strings.Builder
	input.WriteString("load 1m\n")
	for i := 0; i < 10; i++ {
		fmt.Fprintf(&input, "dense_metric{series=%q} 1+1x43199\n", fmt.Sprint(i))
	}
	test, err := NewTest(b, input.String())
	require.NoError(b, err)
	require.NoError(b, test.Run())
	...
	for i := 0; i < b.N; i++ {
		...
		b.ReportMetric(float64(effectivePoints), "effective-points")
		b.ReportMetric(float64(pointCapacity*int(unsafe.Sizeof(Point{}))), "point-capacity-bytes")
		query.Close()
	}
}
