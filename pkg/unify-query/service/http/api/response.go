...
func (r *response) failed(ctx context.Context, err error) {
	...
	metric.APIRequestInc(ctx, r.c.Request.URL.Path, metric.StatusFailed, user.SpaceUid, user.Source, user.Source)
...
func (r *response) success(ctx context.Context, data interface{}) {
	...
	user := metadata.GetUser(ctx)
	metric.APIRequestInc(ctx, r.c.Request.URL.Path, metric.StatusSuccess, user.SpaceUid, user.Source, user.Source)
