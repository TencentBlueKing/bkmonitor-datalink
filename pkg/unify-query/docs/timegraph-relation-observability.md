...
| `cmdb_topology_admission_active` / `cmdb_topology_admission_limit` | 无业务标签；进程内实际占用名额与最近一次准入的有效上限，含代理响应写出期间；limit 在首次请求前为 0 |
...
HTTP 请求 span 与 `scope=request` 耗时均覆盖实际响应写出；通过代理时延迟到代理完成写出及释放后结束。写出失败单独记录为错误，业务失败与编码/传输失败在不同 span 可区分。
...
- `admission_active` 与已有 `inflight` 含义不同：前者按 HTTP 整批请求占一个名额直到响应写完，直接模型调用独占一个；后者计模型调用，嵌套不会给前者重复加一。拒绝不增加 active，取消/失败/重复 release 后归零。瞬时抓取可能错过短峰值，应结合拒绝计数。
...
- `timegraph-admission` / `timegraph-release-admission`：准入是否复用、当前占用与限制，以及实际释放；
