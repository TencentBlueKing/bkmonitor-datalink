...
import (
	...
	"github.com/TencentBlueKing/bkmonitor-datalink/pkg/crontask/utils/stringx"
)
...
func (tsm *TimeSeriesMetric) UpdateMetrics(MetricInfoList []map[string]interface{}) error {
	...
	qs := NewTimeSeriesGroupQuerySet(storage.GlobalDBSession.DB)
	...
	for _, metricInfo := range MetricInfoList {
		...
		if created || lastTimeStr > tsmObj.LastModifyTime.String() {
			qs := NewTimeSeriesMetricQuerySet(storage.GlobalDBSession.DB)
...
func (tsm *TimeSeriesMetric) GetOrCreate() (*TimeSeriesMetric, bool, error) {
	...
	if err := qs.One(&tsmRecord); err != nil {
		created = true
		// create a record
		if err := tsm.Create(storage.GlobalDBSession.DB); err != nil {
