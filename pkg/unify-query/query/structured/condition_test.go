...
func TestConditions_GetCluster(t *testing.T) {
	testCases := []struct {
		Name      string
		Condition Conditions
	}{
		{
			Name: "test normal bcs_cluster",
			Condition: Conditions{
				FieldList: []ConditionField{
					{
						"bcs_cluster_id",
						[]string{"bcs_cluster_id1"},
						ConditionContains,
					}, {
						"bcs_cluster_id",
						[]string{"bcs_cluster_id2"},
						ConditionEqual,
					},
				},
			},
		...
		{
			Name: "test re & normal bcs_cluster",
			Condition: Conditions{
				FieldList: []ConditionField{
					{
						"bcs_cluster_id",
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			conditionFields := testCase.Condition.GetCluster()
			assert.NotNil(t, conditionFields)
			for idx, cond := range conditionFields {
				assert.Equal(t, testCase.Condition.FieldList[idx], cond)
				assert.Equal(t, testCase.Condition.FieldList[idx].Value, cond.Value)
				assert.Equal(t, testCase.Condition.FieldList[idx].Operator, cond.Operator)
				assert.Equal(t, testCase.Condition.FieldList[idx].DimensionName, cond.DimensionName)
			}
		})
	}
}
