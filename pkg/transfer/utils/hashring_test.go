...
func (s *BalanceSuite) TestUsage() {
	...
	for _, c := range cases {
		items := make([]define.IDer, 0, len(c.items))
		for _, i := range c.items {
			items = append(items, bel(i))
		}

		nodes := make([]define.IDer, 0, len(c.nodes))
		for _, i := range c.nodes {
			nodes = append(nodes, bel(i))
		}

		results, _, _ := NewHashBalancer().Balance(define.PlanWithFlows{}, items, nodes)

		for k, values := range c.result {
			mapping := results.All[bel(k)]
...
func (s *DetailsBalanceElementSuite) TestHashing() {
	els1 := NewDetailsBalanceElements("test", 1)
	els2 := NewDetailsBalanceElements("hashing", 1)
...
func (s *DetailsBalanceElementSuite) TestBalance() {
	...
	nodes := []define.IDer{
		NewIDBalanceElement(1),
		NewIDBalanceElement(2),
	}

	items := make([]define.IDer, 0)
	for _, thing := range things {
		els := NewDetailsBalanceElements(thing.name, thing.count)
		items = append(items, els...)
	}

	result, _, _ := NewHashBalancer().Balance(define.PlanWithFlows{}, items, nodes)
	s.NotNil(result)
}
