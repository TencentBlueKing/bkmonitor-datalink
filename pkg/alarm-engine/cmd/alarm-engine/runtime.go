...
import (
	...
	"github.com/TencentBlueKing/bkmonitor-datalink/pkg/alarm-engine/trigger"
)

var (
	ErrApplicationShutdownTimeout = errors.New("alarm engine runtime: shutdown timeout")
	errKafkaServiceStopped        = errors.New("alarm engine runtime: Kafka service stopped unexpectedly")
	errHTTPServiceStopped         = errors.New("alarm engine runtime: HTTP service stopped unexpectedly")
	errFatalWithoutReason         = errors.New("alarm engine runtime: fatal signal has no error")
)
...
func runApplication(ctx context.Context, cfg config.Config, recorder *metric.Recorder, dependencies applicationDependencies) error {
	if ctx == nil {
		return errors.New("alarm engine runtime: context is required")
	}
	if ctx.Err() != nil {
		return nil
	}
	if dependencies.openSink == nil {
		return errors.New("alarm engine runtime: decision sink factory is required")
	}
	if dependencies.openService == nil {
		return errors.New("alarm engine runtime: Kafka service factory is required")
	}
	if dependencies.newHTTP == nil {
		return errors.New("alarm engine runtime: HTTP service factory is required")
	}
