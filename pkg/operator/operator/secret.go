...
func (c *Operator) createOrUpdateEventTaskSecrets() {
	...
	if err != nil {
		logger.Error("could not found any event dataid")
		return
	}
	...
	logger.Infof("event secret [%s] contains file [%s]", secret.Name, eventTarget.FileName())
	...
	if err = k8sutils.CreateOrUpdateSecret(c.ctx, secretClient, secret); err != nil {
		...
		logger.Errorf("failed to create or update event secret %s: %v", secret.Name, err)
		return
	}

	c.mm.IncHandledSecretSuccessCounter(secret.Name, define.ActionCreateOrUpdate)
	logger.Infof("push event secret [%s] into k8s success", secret.Name)
}
...
func (c *Operator) createOrUpdateDaemonSetTaskSecrets(childConfigs []*discover.ChildConfig) {
	...
	for node, configs := range nodeMap {
		...
		if count > maxSecretsAllowed {
			...
			logger.Errorf("daemonset secrets count exceeded, maxAllowed: %v, current: %v", maxSecretsAllowed, count)
			continue
		}
		...
		for _, config := range configs {
			...
			if err != nil {
				logger.Errorf("failed to compress config content, config.addr=%s, err: %v", config.Address, err)
			...
			logger.Debugf("daemonset secret [%s] contains file [%s]", secret.Name, config.FileName)
		}
		...
		c.mm.IncHandledSecretSuccessCounter(secret.Name, define.ActionCreateOrUpdate)
		logger.Infof("push daemonset secret [%s] into k8s success", secret.Name)
	}
	...
	for _, node := range removed {
		...
		if err := secretClient.Delete(c.ctx, secretName, metav1.DeleteOptions{}); err != nil {
			if !errors.IsNotFound(err) {
				...
				logger.Errorf("failed to delete secret [%s], error: %s", secretName, err)
			}
			continue
		}
...
func (c *Operator) createOrUpdateStatefulSetTaskSecrets(childConfigs []*discover.ChildConfig) {
	...
	for idx, configs := range groups {
		...
		if len(cache) > 0 && EqualMap(currTasksCache[idx], cache) {
			...
			logger.Infof("[%s] secrets nothing changed, skipped", secretName)
			continue
		}
		...
		if count > maxSecretsAllowed {
			c.mm.IncSecretsExceededCounter()
			logger.Errorf("statefulset tasks count exceeded, maxAllowed: %v, current: %v", maxSecretsAllowed, count)
			continue
		}
		...
		for _, config := range configs {
			...
			bytesTotal += len(compressed)
			secret.Data[config.FileName] = compressed
			logger.Debugf("statefulset secret [%s] contains file [%s]", secret.Name, config.FileName)
		}
		...
		c.mm.IncHandledSecretSuccessCounter(secret.Name, define.ActionCreateOrUpdate)
		logger.Infof("push statefulset secret [%s] into k8s success", secret.Name)
	}
		...
		if !nextState[prev] {
			...
			if err := secretClient.Delete(c.ctx, prev, metav1.DeleteOptions{}); err != nil {
				if !errors.IsNotFound(err) {
					...
					logger.Errorf("failed to delete secret [%s], error: %s", prev, err)
				}
				continue
			}
