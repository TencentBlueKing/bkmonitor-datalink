...
const lifecycleStepGuides: Record<string, StepGuide> = {
  ...
  final_hook: {
    summary:
      "Alert 发生真实变化时，按 EventSource.hooks 顺序执行插件，分别记录结果。",
...
function lifecycleNodeSnapshot(
  ...
  return [
    { label: "配置入口", value: "EventSource.hooks" },
    { label: "执行顺序", value: "按来源发布配置逐个执行" },
    { label: "失败处理", value: "记录失败后继续后续插件" },
  ];
}
      ...
      >
        <header>
          <div>
            ...
            <span>本地 Console 读取到的脱敏有效配置</span>
          </div>
          <button
            ref={closeButtonRef}
            type="button"
