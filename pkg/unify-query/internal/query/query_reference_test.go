...
func TestConflict(t *testing.T) {
	...
	for name, c := range map[string]struct {
		...
		//假设黑名单为：[[vmrt1,vmrt2][vmrt3,vmrt4,vmrt5]]
		//测试用例1 不匹配黑名单规则 isConflict为 false
		...
		//gzl：测试用例2 匹配黑名单规则 isConflict为 true
		...
		t.Run(name, func(t *testing.T) {
			VmExpand := ToVmExpand(ctx, c.queryRef)
			isConflict := influxdbRouter.CheckVmRt(VmExpand.ResultTableList) // vm rt 黑名单规则检查
