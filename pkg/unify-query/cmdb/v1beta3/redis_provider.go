...
type RedisSchemaProvider struct {
	...
	resourceDefinitions map[string]map[string]*cmdb.ResourceDefinition
	relationDefinitions map[string]map[string]*cmdb.RelationDefinition
	mu                  sync.RWMutex
...
func NewRedisSchemaProvider(client redis.UniversalClient, opts ...RedisSchemaProviderOption) (*RedisSchemaProvider, error) {
	...
	provider := &RedisSchemaProvider{
		client:              client,
		config:              config,
		resourceDefinitions: make(map[string]map[string]*cmdb.ResourceDefinition),
		relationDefinitions: make(map[string]map[string]*cmdb.RelationDefinition),
		ctx:                 ctx,
		cancel:              cancel,
	}
...
func (rsp *RedisSchemaProvider) loadEntityByKind(kind, namespace, name, jsonData string) error {
	...
	case KindResourceDef:
		var def cmdb.ResourceDefinition
		...
		if _, ok := rsp.resourceDefinitions[normalizedNs]; !ok {
			rsp.resourceDefinitions[normalizedNs] = make(map[string]*cmdb.ResourceDefinition)
		}
		rsp.resourceDefinitions[normalizedNs][name] = &def
		rsp.mu.Unlock()

	case KindRelationDef:
		var def cmdb.RelationDefinition
		if err := json.Unmarshal([]byte(jsonData), &def); err != nil {
			return fmt.Errorf("failed to unmarshal RelationDefinition: %w", err)
		}
		rsp.mu.Lock()
		if _, ok := rsp.relationDefinitions[normalizedNs]; !ok {
			rsp.relationDefinitions[normalizedNs] = make(map[string]*cmdb.RelationDefinition)
		}
...
func (rsp *RedisSchemaProvider) reloadNamespace(ctx context.Context, kind, namespace string) error {
	...
	switch kind {
	case KindResourceDef:
		newMap := make(map[string]*cmdb.ResourceDefinition, len(entities))
		for name, jsonData := range entities {
			var def cmdb.ResourceDefinition
	...
	case KindRelationDef:
		newMap := make(map[string]*cmdb.RelationDefinition, len(entities))
		for name, jsonData := range entities {
			var def cmdb.RelationDefinition
			if unmarshalErr := json.Unmarshal(jsonData, &def); unmarshalErr != nil {
...
func (rsp *RedisSchemaProvider) GetResourceDefinition(namespace, name string) (*cmdb.ResourceDefinition, error) {
	var err error
	...
	err = cmdb.ErrResourceDefinitionNotFound
...
func (rsp *RedisSchemaProvider) ListResourceDefinitions(namespace string) ([]*cmdb.ResourceDefinition, error) {
	rsp.mu.RLock()
	defer rsp.mu.RUnlock()
...
func (rsp *RedisSchemaProvider) GetRelationDefinition(namespace, name string) (*cmdb.RelationDefinition, error) {
	var err error
	...
	err = cmdb.ErrRelationDefinitionNotFound
	span.Set("cache.hit", false)
...
func (rsp *RedisSchemaProvider) ListRelationDefinitions(namespace string) ([]*cmdb.RelationDefinition, error) {
	rsp.mu.RLock()
	defer rsp.mu.RUnlock()
...
func (rsp *RedisSchemaProvider) GetRelationSchema(relationType RelationType) (*RelationSchema, error) {
	...
	for _, nsMap := range rsp.relationDefinitions {
		for _, rd := range nsMap {
			if ToRelationType(rd) == relationType {
