...
func (m *AggregateMethod) ToProm(expr parser.Expr) (parser.Expr, error) {
	...
	if method, ok := AggregateMap[strings.ToLower(m.Method)]; ok {
		log.Infof(context.TODO(), "method->[%s] is aggregate method, will make to AggregateExpr", m.Method)
	...
	log.Infof(context.TODO(), "method->[%s] is call method, will make to call expr.", m.Method)
