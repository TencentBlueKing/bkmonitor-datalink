...
func Stop()                                        { defaultWatcher.Stop() }
func AddPath(path string) (<-chan struct{}, error) { return defaultWatcher.AddPath(path) }
func RemovePath(path string) error                 { return defaultWatcher.RemovePath(path) }
...
func (w *Watcher) AddPath(file string) (<-chan struct{}, error) {
	...
	logger.Infof("file [%s] add into watcher", file)
	return ch, nil
}
...
func (w *Watcher) RemovePath(file string) error {
	...
	if cancel, ok := w.watcherCancel[file]; !ok {
		return fmt.Errorf("file [%s] not watching", file)
	} else {
		cancel()
		delete(w.watcherCancel, file)
	}
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if err != nil {
					logger.Errorf("failed to get mod time of file [%s], error: %s", file, err)
					continue
				}

				if modTime != updatedAt {
					logger.Infof("file [%s] changed, send notify", file)
					ch <- struct{}{}
					updatedAt = modTime
				}
			}
