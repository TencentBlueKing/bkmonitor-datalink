...
// GetKVDataContext 读取 KV，并将取消信号传递到底层 HTTP 请求。
var GetKVDataContext = func(ctx context.Context, path string) ([]byte, error) {
	res, err := globalInstance.GetDataContext(ctx, path)
	if err != nil {
		return nil, err
	}
	if res != nil {
