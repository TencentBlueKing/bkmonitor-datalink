...
func newRedisClient(t *testing.T) {
	// 启动 server
	s := miniredis.RunT(t)
...
func TestPubSubData(t *testing.T) {
	newRedisClient(t)

	channel, msg := "bkm-pubsub-test", "test case"

	sub := rs.Subscribe(channel)
	defer sub.Close()

	err := rs.Publish(channel, msg)
	assert.Nil(t, err)
	...
	for {
		...
		case <-time.After(10 * time.Second):
			t.Errorf("timeout")
			return
		case msgChan := <-sub.Channel():
			realMsg := msgChan.Payload
			assert.Equal(t, msg, realMsg)
			t.Logf("receive message")
			return
		}
	}
}
