...
func TestLifecycleCollectorPullsOneConsistentSnapshot(t *testing.T) {
	...
	for _, want := range []string{
		"bkmonitor_alarmd_ready 1",
		"bkmonitor_alarmd_assigned_claims 2",
		"bkmonitor_alarmd_fatal_total 1",
		"bkmonitor_alarmd_draining 1",
		`bkmonitor_alarmd_drain_total{result="success"} 1`,
		`bkmonitor_alarmd_drain_total{result="timeout"} 2`,
		`bkmonitor_alarmd_drain_total{result="failed"} 3`,
		`bkmonitor_alarmd_drain_total{result="_other"} 4`,
		"bkmonitor_alarmd_inflight_records 3",
		"bkmonitor_alarmd_consumer_lag_records 17",
	} {
	...
	if strings.Contains(got, "bkmonitor_alarmd_consumer_lag_records") {
		t.Fatalf("unknown lag was exported as a real series:\n%s", got)
	}
	if !strings.Contains(got, "bkmonitor_alarmd_ready 0") || !strings.Contains(got, "bkmonitor_alarmd_assigned_claims 1") {
