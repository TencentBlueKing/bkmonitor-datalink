...
func TestParseQueryLabelSelector(t *testing.T) {
	...
	t.Run("and_comma", func(t *testing.T) {
		...
		assert.Equal(t, "scene=log,cluster_id=BCS-K8S-00001", AllConditionsToQueryLabelSelectorString(all))
	})
	t.Run("or_groups", func(t *testing.T) {
		...
		assert.Equal(t, "scene=log or scene=k8s", AllConditionsToQueryLabelSelectorString(all))
	})
	...
	t.Run("nreg", func(t *testing.T) {
		...
		assert.Equal(t, `scene!~"test.*"`, AllConditionsToQueryLabelSelectorString(all))
	})
	...
	t.Run("mixed_ops_and_or", func(t *testing.T) {
		...
		assert.Equal(t, `scene=log,env!=prod or cluster_id=~"bcs-.*"`, AllConditionsToQueryLabelSelectorString(all))
	})
