...
func (r *relationDataIDReporter) getToken() (string, error) {
	...
	if token == "" {
		return "", fmt.Errorf("relation DataID %d token is empty", r.dataID)
	}
	r.token = token
...
func getRelationDataIDToken(dataID uint) (string, error) {
	var groups []struct{ Token string }
	if err := mysql.GetDBSession().DB.Model(&customreport.TimeSeriesGroup{}).
		Select("token").
		Where("bk_data_id = ? AND is_enable = ? AND is_delete = ?", dataID, true, false).
		Limit(2).Scan(&groups).Error; err != nil {
