...
const (
	...
	// Range evaluation frequently overestimates the number of points produced by
	// sparse series. Keep the first allocation small and let append follow actual
	// output instead of reserving every possible evaluation step.
	initialPointsSliceSize = 16
...
func (ev *evaluator) rangeEval(prepSeries func(labels.Labels, *EvalSeriesHelper), funcCall func([]parser.Value, [][]EvalSeriesHelper, *EvalNodeHelper) (Vector, storage.Warnings), exprs ...parser.Expr) (Matrix, storage.Warnings) {
	...
	for ts := ev.startTimestamp; ts <= ev.endTimestamp; ts += ev.interval {
		...
		for _, sample := range result {
			...
			if ss.Points == nil {
				ss.Points = getPointSlice(initialPointSliceSize(numSteps))
			}
			sample.Point.T = ts
	...
	case *parser.Call:
		...
		for i, s := range selVS.Series {
			...
			for ts, step := ev.startTimestamp, -1; ts <= ev.endTimestamp; ts += ev.interval {
				...
				enh.Out = outVec[:0]
				if len(outVec) > 0 {
					// The range-window input is transient and can shrink before
					// the series-level check. Exclude it so this early check only
					// rejects output that will inevitably cross maxSamples.
					stableSamples := ev.currentSamples - len(points)
					if stableSamples+len(ss.Points) >= ev.maxSamples {
						ev.error(ErrTooManySamples(env))
					}
					if ss.Points == nil {
						ss.Points = reuseOrGetPointSlice(previousSeries, numSteps)
					}
...
func initialPointSliceSize(estimated int) int {
	if estimated < initialPointsSliceSize {
		return estimated
	}
	return initialPointsSliceSize
}
...
func getPointSlice(sz int) []Point {
	...
	if p != nil {
		points := p.([]Point)
		// A dense range result must not inflate later sparse or instant queries.
		if cap(points) <= maxPointsSliceSize && (sz == 0 || cap(points) <= 2*sz) {
			return points[:0]
		}
	}
...
func putPointSlice(p []Point) {
	if p == nil || cap(p) > maxPointsSliceSize {
		return
	}
...
func reuseOrGetPointSlice(previous *Series, estimated int) []Point {
	if previous != nil && len(previous.Points) > 0 && cap(previous.Points)-2*len(previous.Points) > 0 {
		points := previous.Points[len(previous.Points):cap(previous.Points)]
		previous.Points = previous.Points[:len(previous.Points):len(previous.Points)]
		return points[:0]
	}
	return getPointSlice(initialPointSliceSize(estimated))
}
