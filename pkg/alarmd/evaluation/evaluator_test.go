...
func requestFixtureForPlan(t *testing.T, plan *strategy.CompiledPlan, records []contract.CanonicalRecordV2, history []execution.StateHistoryPoint) execution.EvaluationRequest {
	...
	consumer := execution.ConsumerRef{Plan: id}
	...
	return execution.EvaluationRequest{Header: execution.InternalExecutionHeader{ExecutionID: "execution", Contract: contractRef, DuePlans: []execution.DuePlan{due}, Requirements: []execution.DataRequirement{requirement}, EffectiveTimeFacts: []execution.BoundEffectiveTimeFact{{Consumer: execution.ConsumerRef{Plan: id, LevelID: 5, HasLevel: true}, SeriesIdentity: series, Fact: facts[0]}}, RequiredPhysicalQueries: []execution.PlannedPhysicalQueryRef{{Digest: "physical", QueryRevision: "query"}}, DeadlineUnixMilli: 200000}, Batch: execution.SeriesExecutionBatch{PhysicalQuery: "physical", QueryRevision: "query", CompletionRef: "provider", Dataset: dataset, Inputs: []execution.NamedInputBinding{binding}, Delivery: execution.SeriesDelivery{PhysicalQuery: "physical", QueryRevision: "query", Series: 1, Records: uint64(len(records)), Digest: "delivery"}}, State: execution.StatePreflightResult{Items: []execution.RuntimeStateView{stateView}}, Gaps: execution.GapLoadResult{Items: []execution.GapGuardSnapshot{{Identity: execution.PlanGapIdentity{Plan: id, StateGeneration: "state-v1"}, Status: execution.GapMissing}}}}
}
