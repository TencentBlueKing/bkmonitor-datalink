...
func (k InfluxdbProxyStorageSvc) GetInfluxdbStorage(influxdbProxyStorageId *uint, proxyClusterName *string, storageClusterId *uint) (*storage.InfluxdbProxyStorage, error) {
	...
	if influxdbProxyStorageId != nil {
		if err := storage.NewInfluxdbProxyStorageQuerySet(mysql.GetDBSession().DB).IDEq(*influxdbProxyStorageId).One(&proxy); err != nil {
			return nil, err
		}
		return &proxy, nil
	}
	...
	if proxyClusterName != nil && storageClusterId != nil {
		if err := storage.NewInfluxdbProxyStorageQuerySet(mysql.GetDBSession().DB).ProxyClusterIdEq(*storageClusterId).InstanceClusterNameEq(*proxyClusterName).One(&proxy); err != nil {
			return nil, err
		}
		return &proxy, nil
	}
	...
	if storageClusterId != nil {
		if err := storage.NewInfluxdbProxyStorageQuerySet(mysql.GetDBSession().DB).ProxyClusterIdEq(*storageClusterId).One(&proxy); err != nil {
			return nil, err
		}
		return &proxy, nil
	} else if proxyClusterName != nil {
		if err := storage.NewInfluxdbProxyStorageQuerySet(mysql.GetDBSession().DB).InstanceClusterNameEq(*proxyClusterName).One(&proxy); err != nil {
			return nil, err
		}
		return &proxy, nil
	} else {
		if err := storage.NewInfluxdbProxyStorageQuerySet(mysql.GetDBSession().DB).IsDefaultEq(true).One(&proxy); err != nil {
			return nil, err
		}
		return &proxy, nil
	}
