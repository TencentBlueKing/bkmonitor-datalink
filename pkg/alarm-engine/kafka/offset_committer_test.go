...
func TestSaramaOffsetCommitterCommitsNextOffsetForCurrentGeneration(t *testing.T) {
	...
	if broker.request.ConsumerGroup != "alarm-engine-shadow" || broker.request.ConsumerGroupGeneration != 1 || broker.request.ConsumerID != "member" {
...
func TestSaramaOffsetCommitterReturnsBrokerAndResponseErrors(t *testing.T) {
	...
	for name, broker := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			committer := &SaramaOffsetCommitter{coordinator: fakeCoordinator{broker: broker}, groupID: "alarm-engine-shadow"}
			session := newFakeSession(context.Background(), &[]string{})
			if err := committer.CommitOffset(context.Background(), session, consumer.Record{Topic: "trigger-input", Partition: 3, Offset: 41}); err == nil {
...
func TestSaramaOffsetCommitterPrefersSessionCancellationDuringCommit(t *testing.T) {
	...
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("CommitOffset() error = %v, want context canceled", err)
	}
}
