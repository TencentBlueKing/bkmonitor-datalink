...
// oneShotWalkDispatcher builds the same synthetic dispatcher as walkDispatcher
// but for a one-shot run, opened through beginGeneration so the run's target
// set is the one the dispatcher would really have.
func oneShotWalkDispatcher(
	readyCapacity, recoveryCapacity int,
	owned map[execution.QueryGroupIdentity]time.Time,
) *phaseTwoRunnerDispatcher {
	dispatcher := walkDispatcher(readyCapacity, recoveryCapacity, owned)
	dispatcher.oneShot, dispatcher.generation = true, 0
	dispatcher.beginGeneration()
	return dispatcher
}
...
// owed a turn it did not get.
func TestOneShotRunKeepsTheReturningRunnerOutOfTheQueueToo(t *testing.T) {
	dispatcher := oneShotWalkDispatcher(2, 4, map[execution.QueryGroupIdentity]time.Time{
		"query-group-a": {},
		"query-group-b": {},
		"query-group-c": {},
	})
	...
	dispatcher.fillQueues(runners, revision)
	if got := queuedNames(dispatcher.normal); len(got) != 2 || got[1] != "query-group-c" {
		t.Fatalf("ready queue holds %v, want query-group-c to take the freed place", got)
	}
	if dispatcher.generation != 1 {
		t.Fatalf("a one-shot run reached generation %d, want 1", dispatcher.generation)
	}
}
