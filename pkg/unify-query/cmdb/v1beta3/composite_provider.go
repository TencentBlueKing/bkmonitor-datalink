...
func (csp *CompositeSchemaProvider) GetResourceDefinition(namespace, name string) (*cmdb.ResourceDefinition, error) {
	...
	for i, provider := range csp.providers {
		...
		if !errors.Is(providerErr, cmdb.ErrResourceDefinitionNotFound) {
			lastErr = providerErr
		}
	}
...
func (csp *CompositeSchemaProvider) ListResourceDefinitions(namespace string) ([]*cmdb.ResourceDefinition, error) {
...
// 按优先级顺序查询各个提供器，返回第一个找到的结果
func (csp *CompositeSchemaProvider) GetRelationDefinition(namespace, name string) (*cmdb.RelationDefinition, error) {
	...
	for i, provider := range csp.providers {
		...
		if !errors.Is(providerErr, cmdb.ErrRelationDefinitionNotFound) {
			lastErr = providerErr
		}
	}
...
// 合并所有提供器的结果，去重（以高优先级为准）
func (csp *CompositeSchemaProvider) ListRelationDefinitions(namespace string) ([]*cmdb.RelationDefinition, error) {
...
func (csp *CompositeSchemaProvider) GetRelationSchema(relationType RelationType) (*RelationSchema, error) {
	...
	for _, provider := range csp.providers {
		...
		if !errors.Is(err, cmdb.ErrRelationDefinitionNotFound) {
			lastErr = err
		}
	}
...
// normalizeNamespaceForDedup 将空串 namespace 归一化为 __all__
// Static Provider 使用空串作为全局 namespace，Redis Provider 使用 "__all__"
// 在去重时需要统一，否则同一条关联/资源会因 namespace 不同而被认为是不同记录
func normalizeNamespaceForDedup(namespace string) string {
	if namespace == "" {
		return NamespaceAll
	}
	return namespace
...
func makeResourceCacheKey(namespace, name string) string {
	return normalizeNamespaceForDedup(namespace) + ":" + name
}
