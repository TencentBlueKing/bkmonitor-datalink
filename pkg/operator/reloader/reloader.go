...
func (r *Reloader) loopWatchFile(path string, ch <-chan struct{}) {
	...
	go func() {
		...
		for {
			...
			case <-ch:
				logger.Infof("path [%s] has changed, send reload signal", path)
				r.reloadBus.Publish()
			}
		}
	}()
...
func (r *Reloader) handleSecretAdd(obj interface{})    { r.handleSecrets(define.ActionAdd, obj) }
func (r *Reloader) handleSecretDelete(obj interface{}) { r.handleSecrets(define.ActionDelete, obj) }
...
func (r *Reloader) syncSecretToFiles(secret *corev1.Secret) error {
	...
	for filename, data := range files {
		...
		if err != nil {
			logger.Errorf("copy file [%s] failed, error: %s", filename, err)
			continue
		}
	...
	for _, f := range alreadyExists {
		if _, ok := set[f.Name()]; !ok {
			if err := os.Remove(filepath.Join(ConfChildConfigPath, f.Name())); err != nil {
				logger.Errorf("failed to remove file [%s]", f.Name())
			}
			deleted = true
		}
	}
	...
	for {
		...
		case <-r.reloadBus.Subscribe():
			if err := r.sendReloadSignal(); err != nil {
				logger.Errorf("[bus] failed to send reload signal, error: %s", err)
			}

		case <-ticker.C:
			if err := r.sendReloadSignal(); err != nil {
				logger.Errorf("[ticker] failed to send reload signal, error: %s", err)
			}
		}
	}
}
...
func (r *Reloader) sendReloadSignal() error {
	...
	if err = process.Signal(syscall.SIGUSR1); err != nil {
		return errors.Wrap(err, "send signal to bkmonitorbeat process failed")
	}
	logger.Infof("notify pid [%d] reload finished", pid)
	return nil
}
