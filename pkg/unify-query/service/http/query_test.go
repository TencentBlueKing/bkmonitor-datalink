...
func TestQueryRawWithScroll_DorisOuter(t *testing.T) {
	type Case struct {
		name    string
		queryTs *structured.QueryTs
				...
				QueryList: []*structured.Query{
					{
						...
						KeepColumns: []string{"level"},
						// 限制最大返回10条数据
						SQL: "SELECT `level`, `dtEventTimeStamp` AS `_timestamp_` FROM `2_bklog_bkunify_query_doris`.doris WHERE `dtEventTimeStamp` >= 1757051309000 AND `dtEventTimeStamp` <= 1757054909000 AND `dtEventTime` >= '2025-09-05 13:48:29' AND `dtEventTime` <= '2025-09-05 14:48:30' AND `thedate` = '20250905' LIMIT 10",
					},
				},
				MetricMerge: "a",
				Start:       "1757051309",
