...
describe("KafkaPage", () => {
  it("distinguishes hook instances that share the same output topic", async () => {
    const data = snapshot();
    const output = data.resources.find((r) => r.kind === "output")!;
    output.eventSourceId = "source-a";
    output.hookName = "first-hook";
    data.resources.push({ ...output, hookName: "second-hook" });
    stubSnapshot(data);
    renderPage("/infrastructure/kafka?tab=output");
    const first = await screen.findByRole("button", { name: /first-hook/ });
    const second = screen.getByRole("button", { name: /second-hook/ });
    fireEvent.click(second);
    expect(second).toHaveAttribute("aria-current", "true");
    expect(first).not.toHaveAttribute("aria-current", "true");
    expect(
      filterKafkaResources(data.resources, {
        keyword: "second-hook",
        status: "all",
      }),
    ).toHaveLength(1);
