...
func TestClientFindInstanceByIdentity(t *testing.T) {
	t.Parallel()
	var gotPath string
	var gotBody []byte
	...
	instance, found, err := client.FindInstance(context.Background(), "system", InstanceQuery{
...
func TestClientRejectsUnboundedAndInvalidAttributeQueries(t *testing.T) {
	t.Parallel()
	...
	queries := []InstanceQuery{
		...
		{ModelCode: rulesHostModel, AttributeFilters: []InstanceAttributeFilter{{Field: "attributes.bk_host_id", Type: InstanceAttributeLong, Value: 167}}},
		{ModelCode: rulesHostModel, AttributeFilters: []InstanceAttributeFilter{{Field: "bk_host_id", Type: "unknown", Value: 167}}},
	}
...
func TestClientBuildsNestedAttributeFilters(t *testing.T) {
	t.Parallel()
	var gotBody []byte
	transport := roundTripFunc(func(request *http.Request) (*http.Response, error) {
	...
	_, _, err = client.FindInstance(context.Background(), "system", InstanceQuery{
		ModelCode: rulesHostModel,
		AttributeFilters: []InstanceAttributeFilter{
			{Field: "bk_host_innerip", Type: InstanceAttributeKeyword, Value: "10.10.28.10"},
			{Field: "bk_cloud_id", Type: InstanceAttributeLong, Value: float64(0)},
		},
	})
	if err != nil {
		t.Fatal(err)
...
func TestClientRejectsInvalidResponseIdentity(t *testing.T) {
	t.Parallel()
	cases := []string{
	...
	for _, source := range cases {
		...
		_, _, err = client.FindInstance(context.Background(), "tenant-a", InstanceQuery{ModelCode: rulesHostModel, InstanceID: "167"})
...
func TestClientRejectsAmbiguousInstance(t *testing.T) {
	t.Parallel()
	...
	_, found, err := client.FindInstance(context.Background(), "tenant-a", InstanceQuery{ModelCode: rulesHostModel, InstanceID: "167"})
	if found || err == nil || !strings.Contains(err.Error(), "multiple") {
		t.Fatalf("found=%t err=%v", found, err)
	}
}

func TestClientReturnsMissing(t *testing.T) {
	t.Parallel()
	transport := roundTripFunc(func(*http.Request) (*http.Response, error) {
		return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"hits":{"hits":[]}}`))}, nil
	})
	...
	_, found, err := client.FindInstance(context.Background(), "tenant-a", InstanceQuery{ModelCode: rulesHostModel, InstanceID: "missing"})
	if err != nil || found {
...
func TestOneModelRejectsPartialSearch(t *testing.T) {
	for _, body := range []string{`{"timed_out":true,"hits":{"hits":[]}}`, `{"_shards":{"failed":1},"hits":{"hits":[]}}`} {
		...
		_, found, err := parseFindInstanceResponse(response, "tenant-a", InstanceQuery{ModelCode: rulesHostModel, InstanceID: "167"})
