...
func TestQuerySourceConditions(t *testing.T) {
	...
	metric, err := q.ToQueryMetric(ctx, "bkcc__2", TsDBs{{TableID: "fta.event", StorageID: "1", StorageType: metadata.ElasticsearchStorageType, DB: "bkfta_event_*_read", Measurement: "__default__", Filters: []querypkg.Filter{{"bk_biz_id": "2"}}}})
	require.NoError(t, err)
	require.Len(t, metric.QueryList, 1)
	require.Equal(t, "bk_biz_id", metric.QueryList[0].RoutingConditions[0][0].DimensionName)
	require.Len(t, metric.QueryList[0].AllConditions[0], 1)
