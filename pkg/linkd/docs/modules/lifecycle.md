...
KAC 插件把相同 Alert 快照转换为 KAC `alarm_collect_topic` 扁平消息；`event_id` 使用
`linkd-` 加 AlertID 维持生命周期关联，`alarm_id` 使用 `linkd-` 加稳定 UUID 兼容 KAC URL 路由。
同一 Alert 快照重试生成相同 `alarm_id`，不同更新时间或 outcome 生成不同记录身份。等级、
