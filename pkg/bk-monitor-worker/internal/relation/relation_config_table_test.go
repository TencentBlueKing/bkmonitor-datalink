...
type mockSchemaConfig struct {
	resources map[string]*relation.ResourceDefinition // key: 资源名称
	relations map[string]*relation.RelationDefinition // key: 关系名称
}
...
type MockSchemaProvider struct {
	resourceDefs map[string]*relation.ResourceDefinition
	relationDefs map[string]*relation.RelationDefinition
}

func NewMockSchemaProvider() *MockSchemaProvider {
	return &MockSchemaProvider{
		resourceDefs: make(map[string]*relation.ResourceDefinition),
		relationDefs: make(map[string]*relation.RelationDefinition),
	}
}

func (m *MockSchemaProvider) AddResourceDefinition(namespace, resourceType string, fields []relation.FieldDefinition) {
	key := fmt.Sprintf("%s:%s", namespace, resourceType)
	m.resourceDefs[key] = &relation.ResourceDefinition{
		Namespace: namespace,
		Name:      resourceType,
		Fields:    fields,
	}
...
func (m *MockSchemaProvider) AddRelationDefinitionWithDirection(namespace, fromResource, toResource string, isDirectional bool) {
	...
	m.relationDefs[key] = &relation.RelationDefinition{
		Namespace:     namespace,
		Name:          name,
...
func (m *MockSchemaProvider) GetResourceDefinition(namespace, resourceType string) (*relation.ResourceDefinition, error) {
	key := fmt.Sprintf("%s:%s", namespace, resourceType)
