...
func (s *TestSuite) SetupTest() {
	...
	s.router = SetSpaceTsDbRouter("spacetsdb_test.db", "spacetsdb_test", "bkmonitorv3:spaces")
	err := s.router.InitRouter(s.ctx)
	if err != nil {
		panic(err)
	}
}
...
func (s *TestSuite) TestReloadBySpaceKey() {
	...
	fmt.Printf("Space02: %v\n", space)
	time.Sleep(1 * time.Second)
	space = router.GetSpace(s.ctx, "bkcc__2222")
	fmt.Printf("Space03: %v\n", space)
