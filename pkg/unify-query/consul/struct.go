...
// GetDataContext 获取指定路径，并允许调用方取消正在进行的 Consul 请求。
func (i *Instance) GetDataContext(ctx context.Context, path string) (*api.KVPair, error) {
	result, _, err := i.client.KV.Get(path, (&api.QueryOptions{}).WithContext(ctx))
	return result, err
}
