...
// NewLogFormatter
func NewLogFormatter(ctx context.Context, name string) (*LogFormatter, error) {
	...
	return &LogFormatter{
		Processor: NewProcessor(ctx, name, RecordHandlers{
			...
			schema.TransformMetricsHandlerCreator(option.GetOrDefault(config.PipelineConfigOptAllowMetricsMissing, false).(bool)),
			schema.TransformDimensionsHandlerCreator(option.GetOrDefault(config.PipelineConfigOptAllowDimensionsMissing, false).(bool)),
...
func init() {
	define.RegisterDataProcessor("log_format", func(ctx context.Context, name string) (define.DataProcessor, error) {
		...
		return NewLogFormatter(ctx, pipe.FormatName(rt.FormatName(name)))
	})
}
