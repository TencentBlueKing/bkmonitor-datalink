...
func HandlerQueryRawWithScroll(c *gin.Context) {
	...
	log.Infof(ctx, "[DEBUG] Starting queryRawWithScroll with queryTs: %+v", queryTs)

	listData.Total, listData.List, listData.ResultTableOptions, err = queryRawWithScroll(ctx, queryTs)
	if err != nil {
		log.Errorf(ctx, "[DEBUG] queryRawWithScroll failed with error: %v", err)
		listData.Status = &metadata.Status{
			Code:    metadata.QueryRawError,
			Message: err.Error(),
		}
	...
	log.Infof(ctx, "[DEBUG] queryRawWithScroll completed - Total: %d, List length: %d, ResultTableOptions: %+v",
		listData.Total, len(listData.List), listData.ResultTableOptions)
