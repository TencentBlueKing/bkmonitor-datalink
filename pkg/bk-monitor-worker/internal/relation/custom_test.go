...
func TestReportCustomRelation(t *testing.T) {
	...
	t.Run("标签解析错误测试", func(t *testing.T) {
		patches := gomonkey.NewPatches()
		defer patches.Reset()
		...
		testData := []relation.CustomRelationStatus{
			{
				...
				Name:         "test-relation-invalid-json",
				Labels:       `invalid json`, // 无效的JSON
				FromResource: "service",
				ToResource:   "pod",
			},
			{
		...
		// 只有有效的记录会被上报
		assert.Equal(t, 1, len(reportedMetrics))
	})

	t.Run("空标签测试", func(t *testing.T) {
		patches := gomonkey.NewPatches()
		defer patches.Reset()
		...
		testData := []relation.CustomRelationStatus{
			{
				ID:           1,
				Namespace:    "test-namespace",
				Name:         "test-relation-empty-labels",
				Labels:       "", // 空标签
				FromResource: "service",
				ToResource:   "pod",
			},
