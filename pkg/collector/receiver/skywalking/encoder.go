...
func EncodeTraces(segment *agentv3.SegmentObject, token string, extraAttrs map[string]string) ptrace.Traces {
	...
	utils.InsertString(rs, semconv.AttributeServiceName, segment.GetService())
	utils.InsertString(rs, semconv.AttributeServiceInstanceID, segment.GetServiceInstance())
	utils.InsertString(rs, attributeSkywalkingTraceID, segment.GetTraceId())
	utils.InsertString(rs, attributeDataToken, token)

	// 补充数据字段内容 agentLanguage agentType agentVersion
	for k, v := range extraAttrs {
		utils.InsertString(rs, k, v)
	}
...
func swSpanToSpan(traceID string, segmentID string, span *agentv3.SpanObject, dest ptrace.Span) {
	...
	utils.InsertString(attrs, attributeSkywalkingSegmentID, segmentID)
	...
	case agentv3.SpanLayer_Http:
		...
		if spanType == agentv3.SpanType_Entry {
			...
			if refs := span.Refs; len(refs) > 0 && refs[0].ParentService != "" {
			    utils.InsertString(dest, semconv.AttributeNetPeerName, refs[0].ParentService)
			} else {
				if peerName != "" {
					utils.InsertString(dest, semconv.AttributeNetPeerName, peerName)
				} else {
					utils.InsertString(dest, semconv.AttributeNetPeerName, unknownVal)
				}
			}
		}
		...
		// 获取 urlObj
		var urlObj *url.URL
		if u, ok := dest.Get(semconv.AttributeHTTPURL); ok && u.Str() != "" {
			if v, err := url.Parse(u.Str()); err == nil {
				urlObj = v
			}
		}
		...
		if urlObj != nil {
			...
			if !ok {
				utils.InsertString(dest, semconv.AttributeHTTPScheme, utils.FirstUpper(urlObj.Scheme, unknownVal))
			} else {
				utils.InsertString(dest, semconv.AttributeHTTPScheme, utils.FirstUpper(httpScheme.Str(), unknownVal))
			}
			// attribute.http.target / attribute.http.host
			if spanType == agentv3.SpanType_Exit {
				if _, ok := dest.Get(semconv.AttributeHTTPTarget); !ok {
					utils.InsertString(dest, semconv.AttributeHTTPTarget, urlObj.Path)
				}
				if _, ok := dest.Get(semconv.AttributeHTTPHost); !ok {
					utils.InsertString(dest, semconv.AttributeHTTPHost, urlObj.Host)
				}
			}
		}
	...
	case agentv3.SpanLayer_Database, agentv3.SpanLayer_Cache:
		...
		if span.SpanLayer == agentv3.SpanLayer_Database {
			...
			if v, ok := dest.Get(semconv.AttributeDBSystem); ok {
				dest.PutString(semconv.AttributeDBSystem, utils.FirstUpper(v.Str(), unknownVal))
			}
		}
		// attributes.db.operation
		if _, ok := dest.Get(semconv.AttributeDBOperation); !ok {
			if v, ok := dest.Get(semconv.AttributeDBStatement); ok && v.Str() != "" {
				ops := strings.Split(v.Str(), " ")
				...
				if opName := span.GetOperationName(); opName != "" {
					...
					utils.InsertString(dest, semconv.AttributeDBOperation, ops[len(ops)-1])
				}
			}
		}
...
func setSwSpanIDToAttributes(span *agentv3.SpanObject, dest pcommon.Map) {
	utils.InsertInt(dest, attributeSkywalkingSpanID, int64(span.GetSpanId()))
	if span.ParentSpanId != -1 {
		utils.InsertInt(dest, attributeSkywalkingParentSpanID, int64(span.GetParentSpanId()))
	}
}
...
func swKvPairsToInternalAttributes(pairs []*commonv3.KeyStringValuePair, dest pcommon.Map) {
	...
	for _, pair := range pairs {
		...
		if v, ok := otSpanTagsMapping[pair.Key]; ok {
			utils.InsertString(dest, v, pair.Value)
		} else {
			utils.InsertString(dest, pair.Key, pair.Value)
		}
	}
}
