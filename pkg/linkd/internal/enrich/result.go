...
type ProcessorEnvelope struct {
	...
	Value       domain.JSONObject    `json:"value,omitempty"`
	Patches     []domain.EnrichPatch `json:"patches,omitempty"`
...
func (p Payload) Validate() error {
	...
	for index, entry := range p.Processors {
		...
		for name, envelope := range entry {
			...
			if envelope.Value == nil && envelope.Patches == nil {
...
func DecodePayload(object domain.JSONObject) (Payload, error) {
	...
	for _, entry := range payload.Processors {
		for name, envelope := range entry {
			if envelope.Value == nil && envelope.Patches != nil {
				envelope.Value = processorValueView(name, envelope.Patches)
				entry[name] = envelope
			}
		}
	}
...
// Trace 描述一个规则/操作的执行结果，不包含外部凭据或完整查询响应。
type Trace struct {
...
// MarshalJSON 新结果只序列化补丁；value 仅用于真实历史记录的读取边界。
func (e ProcessorEnvelope) MarshalJSON() ([]byte, error) {
	if e.Patches != nil {
		return json.Marshal(struct {
			...
			Trace       []Trace              `json:"trace,omitempty"`
			Diagnostics []Diagnostic         `json:"diagnostics,omitempty"`
		}{e.Status, e.Patches, e.Trace, e.Diagnostics})
	}
	type legacy ProcessorEnvelope
	return json.Marshal(legacy(e))
}

// processorValueView 是内置类型消费者的临时投影；MarshalJSON 始终以 patches 为唯一新存储格式。
func processorValueView(name string, patches []domain.EnrichPatch) domain.JSONObject {
	var value any = map[string]any{}
	switch name {
	case "resource":
		value = models.ResourceValues{CWLabels: []string{}, DynamicGroupID: []string{}}
	case "display":
	...
	for _, patch := range patches {
		...
		if len(parts) == 2 {
			...
			if name == "display" && key == "subject_name" {
				key = "object"
			}
		}
		if key != "" {
			result[key] = append(json.RawMessage(nil), patch.Value...)
		}
	}
	return result
}
