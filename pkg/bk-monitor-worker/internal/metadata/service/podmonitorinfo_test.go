...
func TestPodMonitorInfoSvc_RefreshResource(t *testing.T) {
	...
	mysql.GetDBSession().DB.Delete(&bcs.PodMonitorInfo{}, "cluster_id = ?", cluster.ClusterID)
	...
	err = bcs.NewPodMonitorInfoQuerySet(mysql.GetDBSession().DB).ClusterIDEq(cluster.ClusterID).All(&results)
	assert.Nil(t, err)
