...
type DirectionType = relation.DirectionType

const (
	DirectionTypeDirectional   = relation.DirectionTypeDirectional
	DirectionTypeBidirectional = relation.DirectionTypeBidirectional
)
...
func (b *MetricsBuilder) buildRelationConfigMetrics(bizID int, info *Info, parentExpands *ParentExpandsStore) []Metric {
	...
	for targetResource, fields := range info.RelationConfig {
		...
		// 1.1 尝试单向关系
		if relationDef, found := schemaProvider.FindRelationByResourceTypes(namespace, targetResource, info.Resource, DirectionTypeDirectional); found {
			foundAny = true
		...
		// 1.2 尝试双向关系
		if relationDef, found := schemaProvider.FindRelationByResourceTypes(namespace, targetResource, info.Resource, DirectionTypeBidirectional); found {
			foundAny = true
			if metric := generateMetric(relationDef, targetResource, fields); metric != nil {
				metrics = append(metrics, *metric)
			}
