...
const (
	cursorKindEvent        = "unprocessed_event"
	cursorKindAllEvent     = "all_unprocessed_event"
	cursorKindEventByAlert = "event_by_alert"
	cursorKindAlertLog     = "alert_log"
)
...
func decodeCursor(encoded string, expected cursorPayload) (cursorPayload, error) {
	...
	if err != nil {
		return cursorPayload{}, fmt.Errorf("%w: decode base64 cursor: %w", store.ErrInvalidCursor, err)
	}
	var decoded cursorPayload
	if err := json.Unmarshal(data, &decoded); err != nil {
		return cursorPayload{}, fmt.Errorf("%w: decode cursor payload: %w", store.ErrInvalidCursor, err)
	}
