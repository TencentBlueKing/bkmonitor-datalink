...
func (e jsonEncoder) UnmarshalTraces(buf []byte) (ptrace.Traces, error) {
	if traces, handled, err := decodeTracesWithTraceID(buf, e.requestTraceID); handled {
		return traces, err
	}
...
func (e jsonEncoder) UnmarshalMetrics(buf []byte) (pmetric.Metrics, error) {
	if metrics, handled, err := decodeMetrics(buf); handled {
		return metrics, err
	}
...
func (e jsonEncoder) UnmarshalLogs(buf []byte) (plog.Logs, error) {
	if logs, handled, err := decodeLogs(buf); handled {
		return logs, err
	}
