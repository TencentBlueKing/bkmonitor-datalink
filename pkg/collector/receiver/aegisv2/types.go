...
func (r *d2Record) UnmarshalJSON(data []byte) error {
	...
	var fs string
	if err := json.Unmarshal(p.Fields, &fs); err != nil {
		return err
	}
...
func (m *d2Message) UnmarshalJSON(data []byte) error {
	...
	*m = d2Message{
		...
		Timestamp:         intField(raw, "timestamp"),
		SessionType:       stringField(raw, "session_type"),
		IsActive:          boolField(raw, "is_active"),
		SessionFrom:       stringField(raw, "session_from"),
		AegisV2Goto:       stringField(raw, "aegisv2_goto"),
		DNSLookup:         intField(raw, "dnsLookup"),
		TCP:               intField(raw, "tcp"),
		SSL:               intField(raw, "ssl"),
		TTFB:              intField(raw, "ttfb"),
		ContentDownload:   intField(raw, "contentDownload"),
		DOMParse:          intField(raw, "domParse"),
		ResourceDownload:  intField(raw, "resourceDownload"),
		FirstScreenTiming: intField(raw, "firstScreenTiming"),
		raw:               raw,
	}
	return nil
}
...
// 新增事件类型时，同步更新 http.go 中的 whitelistSampleMap。
const (
