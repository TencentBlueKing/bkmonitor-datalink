...
func TestRecordingDecisionSinkCountsOutputOnlyAfterAcknowledgement(t *testing.T) {
	...
	if got := counterValue(t, recorder, "bkmonitor_alarmd_records_total", map[string]string{
...
func validApplicationConfig() config.Config {
	return config.Config{
		...
		Kafka: config.KafkaConfig{
			...
			InputTopic:          "alarmd-shadow-input",
			OutputTopic:         "alarmd-shadow-output",
			AllowedOutputTopics: []string{"alarmd-shadow-output"},
			GroupID:             "alarmd-shadow",
			ClientID:            "alarmd",
			BrokerVersion:       "2.6.0",
		},
