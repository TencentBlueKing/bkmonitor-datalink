...
下列配置位于 `cmdb.v1beta3`。这些是普通模式下有限资源保护的默认值，不是吞吐或延迟 SLO。
...
响应字节、Matrix 点数及输出元素/字节拒绝均有固定 reason 指标；
...
这只作用于测试进程。并发档位比较核心流水线成本，不经过公开 HTTP handler，不能据此
承诺生产吞吐或峰值内存。HTTP 批次、活动计数及代理生命周期由另外的回归测试验证。
