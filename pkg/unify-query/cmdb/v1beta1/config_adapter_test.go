...
// mockSchemaProvider 简单 mock 实现 cmdb.SchemaProvider
type mockSchemaProvider struct {
	resources []*cmdb.ResourceDefinition
	relations []*cmdb.RelationDefinition
}

func (m *mockSchemaProvider) GetResourceDefinition(namespace, name string) (*cmdb.ResourceDefinition, error) {
	for _, r := range m.resources {
		if r.Namespace == namespace && r.Name == name {
			return r, nil
		}
	}
	return nil, cmdb.ErrResourceDefinitionNotFound
}

func (m *mockSchemaProvider) ListResourceDefinitions(namespace string) ([]*cmdb.ResourceDefinition, error) {
	result := make([]*cmdb.ResourceDefinition, 0)
	for _, r := range m.resources {
...
func (m *mockSchemaProvider) GetRelationDefinition(namespace, name string) (*cmdb.RelationDefinition, error) {
	for _, r := range m.relations {
		if r.Namespace == namespace && r.Name == name {
			return r, nil
		}
	}
	return nil, cmdb.ErrRelationDefinitionNotFound
}

func (m *mockSchemaProvider) ListRelationDefinitions(namespace string) ([]*cmdb.RelationDefinition, error) {
	result := make([]*cmdb.RelationDefinition, 0)
	for _, r := range m.relations {
		if r.Namespace == namespace || namespace == "" {
			result = append(result, r)
		}
		...
		resources: []*cmdb.ResourceDefinition{
			{
				Namespace: "",
				Name:      "pod",
				Fields:    []cmdb.FieldDefinition{{Name: "pod", Required: true}},
			},
		},
		relations: []*cmdb.RelationDefinition{
			{
...
func TestConfigAdapter_GetConfig_DeduplicateByName(t *testing.T) {
	...
	provider := &mockSchemaProvider{
		resources: []*cmdb.ResourceDefinition{
			{Namespace: "bkcc__2", Name: "pod", Fields: []cmdb.FieldDefinition{{Name: "pod", Required: true}}},
			{Namespace: "bkcc__3", Name: "pod", Fields: []cmdb.FieldDefinition{{Name: "pod", Required: true}}},
			{Namespace: "__all__", Name: "pod", Fields: []cmdb.FieldDefinition{{Name: "pod", Required: true}}},
			{Namespace: "bkcc__2", Name: "node", Fields: []cmdb.FieldDefinition{{Name: "node", Required: true}}},
			{Namespace: "__all__", Name: "node", Fields: []cmdb.FieldDefinition{{Name: "node", Required: true}}},
		},
		relations: []*cmdb.RelationDefinition{
...
func TestConfigAdapter_GetConfig_DeduplicateUndirectedEdge(t *testing.T) {
	...
	provider := &mockSchemaProvider{
		resources: []*cmdb.ResourceDefinition{
			{Namespace: "", Name: "pod", Fields: []cmdb.FieldDefinition{{Name: "pod", Required: true}}},
			{Namespace: "", Name: "node", Fields: []cmdb.FieldDefinition{{Name: "node", Required: true}}},
		},
		relations: []*cmdb.RelationDefinition{
