...
func (s *CCApiClientSuite) TestGetHostsByRange() {
	...
	res, err := s.apiClient.GetHostsByRange("system", 1, 0, 0)
	s.NoError(err)
...
func (s *CCApiClientSuite) TestGetServiceInstance() {
	...
	response, err := s.apiClient.GetServiceInstance("system", 1, 1, 1, []int{1})
	s.NoError(err)
	s.NotNil(response)
...
func (s *CCApiClientSuite) TestTopoDataToCmdbLevelV3() {
	s.doer.EXPECT().Do(gomock.Any()).Return(
		...
		nil)
	response, err := s.apiClient.GetSearchBizInstTopo("system", 0, 2, 0, -1)
	s.NoError(err)
