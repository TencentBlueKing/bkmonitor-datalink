...
func (w *dataIDWatcher) handleDataIDAdd(obj interface{}) {
	w.mm.IncReceivedK8sEventCounter(define.ActionAdd)
	defer w.mm.IncHandledK8sEventCounter(define.ActionAdd)

	dataID, ok := obj.(*bkv1beta1.DataID)
	if !ok {
		logger.Errorf("incorrect type: expect DataID, got %T", obj)
		return
	}
...
func (w *dataIDWatcher) handleDataIDDelete(obj interface{}) {
	w.mm.IncReceivedK8sEventCounter(define.ActionDelete)
	defer w.mm.IncHandledK8sEventCounter(define.ActionDelete)
...
func (w *dataIDWatcher) handleDataIDUpdate(oldObj interface{}, newObj interface{}) {
	w.mm.IncReceivedK8sEventCounter(define.ActionUpdate)
	defer w.mm.IncHandledK8sEventCounter(define.ActionUpdate)

	old, ok := oldObj.(*bkv1beta1.DataID)
	if !ok {
		logger.Errorf("incorrect type: expect DataID, got %T", oldObj)
		return
	}
	cur, ok := newObj.(*bkv1beta1.DataID)
	if !ok {
		logger.Errorf("incorrect type: expect DataID, got %T", newObj)
		return
	}

	if old.ResourceVersion == cur.ResourceVersion {
		w.mm.IncHandledK8sEventCounter(define.ActionSkip)
		return
	}
