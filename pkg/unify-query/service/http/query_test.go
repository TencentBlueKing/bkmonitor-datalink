...
`,
	...
	vmInst, _ := victoriaMetrics.NewInstance(ctx, &victoriaMetrics.Options{
		Address: "127.0.0.1",
		Curl:    mockCurl,
		...
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			if err == nil {
				...
				if c.error != nil {
					...
					if mockCurl.Opts.Body != nil {
						err = json.Unmarshal(mockCurl.Opts.Body, &vmParams)
						assert.Nil(t, err)
					}
					if vmParams != nil {
...
func TestQueryTs_ToQueryReference(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.True(t, metadata.GetQueryParams(ctx).IsVmQuery())
}
