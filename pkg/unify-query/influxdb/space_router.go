...
func reloadAllSpaces(ctx context.Context, spaceUidList []string) error {
	...
	var spaceUidByteList [][]byte
	var spaceDataList [][]byte
	for _, suid := range spaceUidList {
		...
		spaceDataList = append(spaceDataList, v)
		spaceUidByteList = append(spaceUidByteList, kvstore.String2byte(suid))
	}
	// 批量写入
	if err := spaceRouter.kvClient.BatchWrite(spaceUidByteList, spaceDataList); err != nil {
