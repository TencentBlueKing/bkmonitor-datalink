		...
		if len(ipListTmp) == 0 {
			if ipType == configs.IPv4 {
				hostsInfo = &HostInfo{
					Host:  domain,
					Ips:   nil,
					Errno: define.CodeNetResponseNotFindIpv4Err,
				}
			}
			if ipType == configs.IPv6 {
				hostsInfo = &HostInfo{
					Host:  domain,
					Ips:   nil,
					Errno: define.CodeNetResponseNotFindIpv6Err,
				}
			}
		}
	}
		...
		if protocol == configs.Http {
			...
			if err != nil {
				hostsInfo = append(hostsInfo, HostInfo{
					Host:  hostSrc,
					Ips:   nil,
					Errno: define.CodeNetResponseParseUrlErr,
				})
				continue
			}
			host = u.Host
		...
		if utils.CheckIpOrDomainValid(host) == utils.Domain {
			...
			if err != nil {
				...
				hostsInfo = append(hostsInfo, HostInfo{
					Host:  hostSrc,
					Ips:   nil,
					Errno: define.CodeDNSResolveErr,
				})
				continue
			}
			for _, v := range ips {
