...
// CheckVmRt 实现黑名单检查逻辑
func (r *Router) IsCheckVmBlock(ctx context.Context, vmClusterNames *set.Set[string]) bool {
	r.lock.RLock()
	defer r.lock.RUnlock()
	...
	var err error
	ctx, span := trace.NewSpan(ctx, "is-check-vm-block")
	defer span.End(&err)

	var forbiddenVmCluster [][]string

	forbiddenVmCluster = r.blackListInfo.ForbiddenVmCluster

	// 遍历该配置下的所有禁止规则
	for _, ruleTable := range forbiddenVmCluster {
		if len(ruleTable) == 0 {
			continue
		}
		...
		for _, rule := range ruleTable {
			if !vmClusterNames.Existed(rule) {
