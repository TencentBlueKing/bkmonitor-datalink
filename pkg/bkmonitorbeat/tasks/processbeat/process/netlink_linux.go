...
loop:
	for {
		...
		for _, netlinkMessage := range msgs {
...
// getIntersection inode -> socket
func getIntersection(conn map[uint32]FileSocket, inodes map[int32][]uint64, tcp bool) map[uint32]FileSocket {
	ret := make(map[uint32]FileSocket)
...
func getPidSockets(sni socketNetInfo, pids []int32) PidSockets {
	var socketnet socketNetInfo
	inodes := getConcernPidInodes(pids) // pid -> inodes
	socketnet.TCP = getIntersection(sni.TCP, inodes, true)
	socketnet.UDP = getIntersection(sni.UDP, inodes, false)
	socketnet.TCP6 = getIntersection(sni.TCP6, inodes, true)
	socketnet.UDP6 = getIntersection(sni.UDP6, inodes, false)
...
var _ ConnDetector = NetlinkConnector{}

func (NetlinkConnector) Get(pids []int32) (PidSockets, error) {
	type Task struct {
		proto      uint8
		family     uint8
		states     uint32
	...
	for _, t := range tasks {
		if taskFd, err = sockdiagSend(t.proto, 0, t.family, 0, t.states); err != nil {
			logger.Errorf("sockdiag send failed, err: %v", err)
			continue
		}
	...
	return getPidSockets(cni, pids), err
}
