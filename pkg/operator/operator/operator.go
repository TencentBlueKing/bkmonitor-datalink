...
func (c *Operator) reloadAllDiscovers() {
	...
	for name, dis := range c.discovers {
		...
		if err != nil {
			logger.Errorf("meta=%v, dis=%s found no dataid", meta, dis)
			continue
		}
		...
		if err := dis.Reload(); err != nil {
			logger.Errorf("discover [%s] reload failed", name)
		}
	}
}
...
func (c *Operator) createServiceMonitorDiscovers(serviceMonitor *promv1.ServiceMonitor) []discover.Discover {
	...
	logger.Infof("get serviceMonitor from k8s, name=%s, namespace=%s", serviceMonitor.Name, serviceMonitor.Namespace)
	for index, endpoint := range serviceMonitor.Spec.Endpoints {
		...
		if err != nil {
			logger.Errorf("failed to get convert relabels, error: %s", err)
			continue
		}
...
func (c *Operator) handleServiceMonitorAdd(obj interface{}) {
	...
	if !ok {
		logger.Errorf("incorrect type: expect ServiceMonitor, got %T", obj)
		return
	}
	...
	now := time.Now()
	c.mm.IncReceivedK8sEventCounter(serviceMonitor.Kind, define.ActionAdd)
	...
	for _, dis := range discovers {
		if err := c.addOrUpdateDiscover(dis); err != nil {
			logger.Errorf("failed to add or update serviceMonitor discover, error: %s", err)
		}
	}
	c.mm.IncHandledK8sEventCounter(serviceMonitor.Kind, define.ActionAdd)
	c.mm.ObserveHandledK8sEventDuration(now, serviceMonitor.Kind, define.ActionAdd)
}

func (c *Operator) handleServiceMonitorUpdate(oldObj interface{}, newObj interface{}) {
	old, ok := oldObj.(*promv1.ServiceMonitor)
	if !ok {
		logger.Errorf("incorrect type: expect ServiceMonitor, got %T", oldObj)
		return
	}
	...
	if !ok {
		logger.Errorf("incorrect type: expect ServiceMonitor, got %T", newObj)
		return
	}
	...
	now := time.Now()
	c.mm.IncReceivedK8sEventCounter(old.Kind, define.ActionUpdate)
	...
	c.mm.IncHandledK8sEventCounter(old.Kind, define.ActionUpdate)
	c.mm.ObserveHandledK8sEventDuration(now, old.Kind, define.ActionUpdate)
}
...
func (c *Operator) handleServiceMonitorDelete(obj interface{}) {
	...
	now := time.Now()
	c.mm.IncReceivedK8sEventCounter(serviceMonitor.Kind, define.ActionDelete)
	for _, name := range c.getServiceMonitorDiscoversName(serviceMonitor) {
		c.deleteDiscoverByName(name)
	}
	c.mm.IncHandledK8sEventCounter(serviceMonitor.Kind, define.ActionDelete)
	c.mm.ObserveHandledK8sEventDuration(now, serviceMonitor.Kind, define.ActionDelete)
}
...
func (c *Operator) createPodMonitorDiscovers(podMonitor *promv1.PodMonitor) []discover.Discover {
	...
	logger.Infof("get podMonitor from k8s, name=%s, namespace=%s", podMonitor.Name, podMonitor.Namespace)
	for index, endpoint := range podMonitor.Spec.PodMetricsEndpoints {
		...
		resultLabels, err := convertYamlRelabels(relabels)
		if err != nil {
			logger.Errorf("failed to convert relabels, error: %s", err)
			continue
		}
...
func (c *Operator) handlePodMonitorAdd(obj interface{}) {
	...
	if !ok {
		logger.Errorf("incorrect type: expect PodMonitor, got %T", obj)
		return
	}
	...
	now := time.Now()
	c.mm.IncReceivedK8sEventCounter(podMonitor.Kind, define.ActionAdd)
	discovers := c.createPodMonitorDiscovers(podMonitor)
	for _, dis := range discovers {
		if err := c.addOrUpdateDiscover(dis); err != nil {
			logger.Errorf("failed to update podMonitor discover, error: %s", err)
		}
	}
	c.mm.IncHandledK8sEventCounter(podMonitor.Kind, define.ActionAdd)
	c.mm.ObserveHandledK8sEventDuration(now, podMonitor.Kind, define.ActionAdd)
}

func (c *Operator) handlePodMonitorUpdate(oldObj interface{}, newObj interface{}) {
	old, ok := oldObj.(*promv1.PodMonitor)
	if !ok {
		logger.Errorf("incorrect type: expect PodMonitor, got %T", oldObj)
		return
	}
	...
	if !ok {
		logger.Errorf("incorrect type: expect PodMonitor, got %T", newObj)
		return
	}
	...
	for _, dis := range c.createPodMonitorDiscovers(cur) {
		if err := c.addOrUpdateDiscover(dis); err != nil {
			logger.Errorf("failed to add or update podMonitor discover, error: %s", err)
		}
	}
	c.mm.IncHandledK8sEventCounter(old.Kind, define.ActionUpdate)
	c.mm.ObserveHandledK8sEventDuration(now, old.Kind, define.ActionUpdate)
...
func (c *Operator) handlePodMonitorDelete(obj interface{}) {
	...
	now := time.Now()
	c.mm.IncReceivedK8sEventCounter(podMonitor.Kind, define.ActionDelete)
	for _, name := range c.getPodMonitorDiscoversName(podMonitor) {
		c.deleteDiscoverByName(name)
	}
	c.mm.IncHandledK8sEventCounter(podMonitor.Kind, define.ActionDelete)
	c.mm.ObserveHandledK8sEventDuration(now, podMonitor.Kind, define.ActionDelete)
}
