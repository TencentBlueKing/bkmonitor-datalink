...
func putCommonSpanAttrs(attrs pcommon.Map, payload collectPayload, record d2Record) {
...
// putAssetSpeedSpanAttrs 填充静态资源加载（assets_speed）Span 的扩展属性。
func putAssetSpeedSpanAttrs(attrs pcommon.Map, payload collectPayload, record d2Record, msg d2Message) {
	...
	if resourceURL != "" {
		...
		putTargetURLAttrs(attrs, resourceURL)
	}
	putPageAttrs(attrs, record.Fields.View.ViewURL)
	if hasStatusCode {
	...
	if isErr, ok := extractBool(msg.raw, "isErr"); ok && isErr {
		putResultAttrs(attrs, "error", aegisEvent{record, msg}.ExceptionType())
		return
	}
	if hasStatusCode && statusCode >= 400 {
		putResultAttrs(attrs, "error", httpStatusClass(statusCode))
		return
	}
	putResultAttrs(attrs, "success", "none")
}

func putPagePerformanceSpanAttrs(attrs pcommon.Map, record d2Record) {
...
func putWebsocketSpanAttrs(attrs pcommon.Map, payload collectPayload, record d2Record, msg d2Message) {
	...
	if endpointURL := firstNonEmptyString(msg.raw, "url"); endpointURL != "" {
		...
		putTargetURLAttrs(attrs, endpointURL)
	}

	putPageAttrs(attrs, recordPageURL(record))
	...
	if success, ok := extractBool(msg.raw, "successFlag"); ok {
		...
		putResultAttrs(attrs, "error", event.ExceptionType())
		return
	}
...
func putTargetURLAttrs(attrs pcommon.Map, rawURL string) {
...
func putPageAttrs(attrs pcommon.Map, rawURL string) {
	if rawURL != "" {
		putPageURLAttrs(attrs, rawURL)
	}
}

func putFullPageAttrs(attrs pcommon.Map, rawURL string) {
	if rawURL != "" {
...
func putBrowserContextAttrs(attrs pcommon.Map, payload collectPayload) {
	if payload.Bean.NetType != "" {
		upsertString(attrs, "network.effective_type", payload.Bean.NetType)
	}
	putViewportAttrs(attrs, payload.Bean.VP, "browser.viewport")
	putViewportAttrs(attrs, payload.Bean.SR, "browser.screen")
}

func putResultAttrs(attrs pcommon.Map, result, errorType string) {
	upsertString(attrs, "result", result)
	upsertString(attrs, "error_type", errorType)
}

func putPageURLAttrs(attrs pcommon.Map, rawURL string) {
	parsed, err := url.Parse(rawURL)
	if err != nil {
...
func putViewportAttrs(attrs pcommon.Map, rawValue, prefix string) {
...
func putSpanException(span ptrace.Span, attrs pcommon.Map, exceptionType, message string) {
