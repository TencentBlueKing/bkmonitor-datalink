...
// Index facts keep their closed vocabulary: an unknown read result
// normalizes to invalid, so a producer bug reads as "do not trust this"
// rather than as a new category, and negative counts clamp to zero.
func TestAssignmentIndexFactsKeepClosedVocabularies(t *testing.T) {
	got := NormalizeObservation(Observation{
		...
		AssignmentIndex: &AssignmentIndexFacts{Round: 3, Result: "sideways", StaleRounds: -4, Candidates: 2, Assigned: -1, Opened: -2, Reads: -3},
	})
	...
	if facts.Result != AssignmentIndexInvalid || facts.StaleRounds != 0 || facts.Assigned != 0 ||
		facts.Opened != 0 || facts.Reads != 0 || facts.Candidates != 2 || facts.Round != 3 {
