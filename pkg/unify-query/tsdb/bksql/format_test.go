...
func TestQueryFactory_SearchAfterUsesInternalSortColumns(t *testing.T) {
	query := &metadata.Query{
		DB:            "log_table",
		...
		ResultTableOption: &metadata.ResultTableOption{
			SearchAfter: []any{json.Number("17"), "log-1"},
		},
	}
	factory := bksql.NewQueryFactory(metadata.InitHashID(context.Background()), query).
		...
		WithKeepColumns(query.Source)

	sql, err := factory.SQL()
	require.NoError(t, err)
	assert.Contains(t, sql, "`gseIndex` AS `__search_after_0`")
	assert.Contains(t, sql, "`__unique_key__` AS `__search_after_1`")
	assert.Contains(t, sql, "((`gseIndex` > 17) OR (`gseIndex` = 17 AND `__unique_key__` > 'log-1'))")
	assert.Contains(t, sql, "ORDER BY `gseIndex` ASC, `__unique_key__` ASC")

	values, err := factory.SearchAfterValues(map[string]any{
		"message":          "next page",
		"__search_after_0": json.Number("18"),
		"__search_after_1": "log-2",
	})
	require.NoError(t, err)
	assert.Equal(t, []any{json.Number("18"), "log-2"}, values)

	result := factory.ReloadListData(map[string]any{
		"message":          "next page",
		"__search_after_0": json.Number("18"),
		"__search_after_1": "log-2",
	}, false)
	assert.Equal(t, map[string]any{"message": "next page"}, result)
	assert.Equal(t, []map[string]any{
		{"field_alias": "message"},
	}, factory.FilterResultSchema([]map[string]any{
		{"field_alias": "message"},
...
func TestQueryFactory_SearchAfterRequiresStableTieBreaker(t *testing.T) {
	query := &metadata.Query{
		DB:            "log_table",
		Measurement:   sql_expr.Doris,
		Field:         "value",
	...
	_, err := factory.SQL()
	assert.EqualError(t, err, "search_after requires __unique_key__ as a stable tie-breaker")
}

func TestQueryFactory_SearchAfterRejectsDistinctQuery(t *testing.T) {
	query := &metadata.Query{
		DB:             "log_table",
		Measurement:    sql_expr.Doris,
		Field:          "value",
		IsSearchAfter:  true,
		SelectDistinct: []string{"message"},
		Orders: metadata.Orders{
			{Name: "gseIndex", Ast: true},
		},
	}
	...
	_, err := factory.SQL()
	assert.EqualError(t, err, "search_after does not support distinct query")
}
...
//
// 子测试 without_fields_map：仅调用 NewQueryFactory（未 WithFieldsMap）时，DorisSQLExpr.dimTransform 对「表结构中未知」的列会改写为 NULL，
// golden 中大量 NULL AS … 是这一单元路径的预期，不等于线上 Instance.InitQueryFactory 在注入 FieldsMap 后的行为。
// 子测试 with_fields_map：模拟 InitQueryFactory 已注入 SHOW CREATE 得到的字段映射时，业务列应为反引号标识符，而非因缺映射而产生的 NULL AS。
...
func TestNewQueryFactory_BkSql_TSpider_UserSQL(t *testing.T) {
	...
	t.Run("with_fields_map", func(t *testing.T) {
		...
		withMapTests := []struct {
			...
			{
				name:    "dtEventTimeStamp_resolved",
				userSQL: "SELECT dtEventTimeStamp FROM tbl WHERE dtEventTimeStamp > 0 LIMIT 10",
				wantSQL: "SELECT `dtEventTimeStamp` FROM `132_lol_new_login_queue_login_1min` WHERE `dtEventTimeStamp` > 0 AND " + whereTime + " LIMIT 10",
			},
			{
				name:    "serverIp_resolved",
				userSQL: "SELECT serverIp FROM tbl WHERE 1 = 1 LIMIT 5",
				wantSQL: "SELECT `serverIp` FROM `132_lol_new_login_queue_login_1min` WHERE 1 = 1 AND " + whereTime + " LIMIT 5",
			},
		}
		for _, tt := range withMapTests {
			t.Run(tt.name, func(t *testing.T) {
				q := &metadata.Query{
					StorageType: metadata.BkSqlStorageType,
					DB:          "132_lol_new_login_queue_login_1min",
				...
				got, err := bksql.NewQueryFactory(ctx, q).WithRangeTime(start, end).WithFieldsMap(fieldsMap).SQL()
				require.NoError(t, err)
				require.NotEmpty(t, got)
				assert.Equal(t, tt.wantSQL, got)
			})
