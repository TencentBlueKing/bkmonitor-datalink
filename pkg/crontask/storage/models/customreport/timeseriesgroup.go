...
import (
	...
	"github.com/TencentBlueKing/bkmonitor-datalink/pkg/crontask/storage/models"
	"github.com/TencentBlueKing/bkmonitor-datalink/pkg/crontask/storage/models/resulttable"
	"github.com/TencentBlueKing/bkmonitor-datalink/pkg/crontask/utils/mapx"
...
func setTimeSeriesMetricDefault() {
...
func (ts *TimeSeriesGroup) GetRedisData() ([]map[string]interface{}, error) {
	...
	redisClient := storage.GlobalRedisSession.Client
	ctx := context.Background()
	// 根据过滤参数，获取总量
	zcount := redisClient.ZCount(ctx, metricKey, validBeginTimeStampStr, nowTimeStampStr)
	zcountVal, err := zcount.Result()
	if err != nil {
	...
	for i := 0; float64(i) < ceilCount; i++ {
		...
		metricsWithScores, err := redisClient.ZRangeByScoreWithScores(ctx, metricKey, &opt).Result()
		...
		dimensions, err := redisClient.HMGet(ctx, metricDimensionsKey, fields...).Result()
		if err != nil {
