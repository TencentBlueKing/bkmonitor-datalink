...
func fetchDbmMetaResponseStore(record *define.ETLRecord, store define.Store) (string, error) {
...
func fetchCCTopoResponse(record *define.ETLRecord, store define.Store) (*models.CCTopoBaseModelInfo, string, error) {
	...
	if isHostInfo {
		if obj, ok := modelInfo.(*models.CCHostInfo); ok {
			dbmMeta = obj.DbmMeta
		}
	}
...
func TransferRecordCutterByDbmMetaCreator(store define.Store, enable bool) define.ETLRecordChainingHandler {
	if !enable {
		return nil
	}
