...
func TestQueryRawWithHandler(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"test_1": {
			...
			expected: `{"total":20,"list":[{"__data_label":"","__doc_id":"4","__index":"","__result_table":"nano.nano","_time":"1764065418831","dtEventTimeStamp":"2025-11-25T12:07:38.747332000Z","dtEventTimeStampNanos":"2025-11-25T12:07:38.747332000Z"},{"__data_label":"","__doc_id":"2","__index":"","__result_table":"nano.millisecond","_time":"2025-11-25T12:07:37.747332000Z","dtEventTimeStamp":"2025-11-25T12:07:37.747332000Z"}],"done":false,"status":null,"result_table_options":{"nano.millisecond|3":{"from":0},"nano.nano|3":{"from":0}},"route_info":[{"reference_name":"a","metric_name":"dtEventTimeStamp","table_id":"nano.millisecond","db":"es_index","data_source":"bklog","storage_type":"elasticsearch","storage_id":"3"},{"reference_name":"a","metric_name":"dtEventTimeStamp","table_id":"nano.nano","db":"es_index_1","data_source":"bklog","storage_type":"elasticsearch","storage_id":"3"}]}`,
		},
	}
...
func TestPromQLQueryHandler(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"test_query_vm_1 and not time align": {
			...
			step:         "10m",
			notTimeAlign: true,
			expected:     `{"series":[{"name":"_result0","metric_name":"","columns":["_time","_value"],"types":["float","float"],"group_keys":["bcs_cluster_id"],"group_values":["BCS-K8S-00000"],"stat":{"count":[0,7],"sum":[0,14132],"min":[1741058843000,1978],"max":[1741057043000,2056],"avg":[0,2018.857142857143],"last":[1741060043000,2052]},"values":[[1741056443000,2042],[1741057043000,2056],[1741057643000,1995],[1741058243000,2008],[1741058843000,1978],[1741059443000,2001],[1741060043000,2052]]}],"is_partial":false,"route_info":[{"reference_name":"a","metric_name":"container_cpu_usage_seconds_total","table_id":"result_table.vm","data_label":"vm","data_source":"bkmonitor","storage_type":"victoria_metrics","storage_id":"2","measurement":"container_cpu_usage_seconds_total"}]}`,
		},
		"test_query_vm_2 and instant": {
			...
			step:     "30m",
			instant:  true,
			expected: `{"series":[{"name":"_result0","metric_name":"","columns":["_time","_value"],"types":["float","float"],"group_keys":["bcs_cluster_id"],"group_values":["BCS-K8S-00000"],"stat":{"count":[0,1],"sum":[0,1172],"min":[1729608144000,1172],"max":[1729608144000,1172],"avg":[0,1172],"last":[1729608144000,1172]},"values":[[1729608144000,1172]]}],"is_partial":false,"route_info":[{"reference_name":"a","metric_name":"kube_pod_info","table_id":"result_table.influxdb","db":"result_table","data_label":"influxdb","data_source":"bkmonitor","storage_type":"influxdb","storage_id":"2","measurement":"kube_pod_info"},{"reference_name":"a","metric_name":"kube_pod_info","table_id":"result_table.vm","data_label":"vm","data_source":"bkmonitor","storage_type":"victoria_metrics","storage_id":"2","measurement":"kube_pod_info"}]}`,
		},
		"test promql bkdata": {
			handler:  HandlerQueryPromQL,
			promql:   `sum(increase({__name__=~"bkdata:.*trace.*:span_name", span_name=~"handler-query-.*"}[3h])) by (span_name)`,
			step:     "3h",
			expected: `{"series":[],"is_partial":false,"route_info":[]}`,
		},
