...
func Test_parseExprToKeyValue(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				query: "a5kS5ByNUabDnQjSBNTbf9SIUXGmJZIxRPbB8I-opGqHzemshvkVRGs7F",
				expr:  nil,
				kv:    map[string][]string{"XEmDv": {"IZZpypR2E"}},
			},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := parseExprToKeyValue(tt.args.query, tt.args.expr, tt.args.kv); (err != nil) != tt.wantErr {
...
func Test_addValueToMap(t *testing.T) {
	type args struct {
		kv    map[string][]string
		field string
		value string
	...
	tests := []struct {
		...
		{
			name: "test_addValueToMap_success",
			args: args{
				kv:    map[string][]string{"test": []string{"test"}},
				field: "test",
				value: "test",
			},
			wantErr: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := addValueToMap(tt.args.kv, tt.args.field, tt.args.value); (err != nil) != tt.wantErr {
				t.Errorf("addValueToMap() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
