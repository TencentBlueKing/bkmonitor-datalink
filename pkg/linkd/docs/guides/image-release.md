...
从产物 index digest 中解析各架构的子 manifest digest，分别以 `--platform linux/amd64` 和 `--platform linux/arm64` 拉起无网络版本查询，
...
例如 `helm/linkd/v0.1.3`；版本直接读取 `Chart.yaml`，没有单独版本输入。
首次创建的 Git tag 指向首次发布提交，标题为 `Linkd Helm Chart 0.1.3`。
...
Chart 包默认沿用仓库的 `version: 0.1.3`、`appVersion: "0.1.3"` 和两个 GHCR `0.1.3` 镜像。
...
helm upgrade --install linkd ./linkd-0.1.3.tgz \
