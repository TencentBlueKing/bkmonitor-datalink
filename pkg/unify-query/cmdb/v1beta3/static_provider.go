...
type StaticSchemaProvider struct {
	resourceDefinitions map[string]*cmdb.ResourceDefinition // key: name
	relationDefinitions map[string]*cmdb.RelationDefinition // key: name
...
func NewStaticSchemaProvider() *StaticSchemaProvider {
	provider := &StaticSchemaProvider{
		resourceDefinitions: make(map[string]*cmdb.ResourceDefinition),
		relationDefinitions: make(map[string]*cmdb.RelationDefinition),
	...
	for resourceType, keys := range resourcePrimaryKeys {
		fields := make([]cmdb.FieldDefinition, len(keys))
		for i, key := range keys {
			fields[i] = cmdb.FieldDefinition{
				Name:     key,
				Required: true,
			}
		}
...
func (sp *StaticSchemaProvider) GetResourceDefinition(namespace, name string) (*cmdb.ResourceDefinition, error) {
	sp.mu.RLock()
	defer sp.mu.RUnlock()
...
func (sp *StaticSchemaProvider) ListResourceDefinitions(namespace string) ([]*cmdb.ResourceDefinition, error) {
	sp.mu.RLock()
	defer sp.mu.RUnlock()

	// 静态提供器只支持全局资源(namespace = "")
	if namespace != "" {
		return []*cmdb.ResourceDefinition{}, nil
	}

	result := make([]*cmdb.ResourceDefinition, 0, len(sp.resourceDefinitions))
...
func (sp *StaticSchemaProvider) GetRelationDefinition(namespace, name string) (*cmdb.RelationDefinition, error) {
	sp.mu.RLock()
	defer sp.mu.RUnlock()
...
func (sp *StaticSchemaProvider) ListRelationDefinitions(namespace string) ([]*cmdb.RelationDefinition, error) {
	sp.mu.RLock()
	defer sp.mu.RUnlock()

	// 静态提供器只支持全局关联(namespace = "")
	if namespace != "" {
		return []*cmdb.RelationDefinition{}, nil
	}

	result := make([]*cmdb.RelationDefinition, 0, len(sp.relationDefinitions))
