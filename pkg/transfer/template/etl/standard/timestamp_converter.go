...
func NewTimestampConvertProcessor(ctx context.Context, name string) (*TimestampConvertProcessor, error) {
...
func init() {
	define.RegisterDataProcessor("timestamp_converter", func(ctx context.Context, name string) (define.DataProcessor, error) {
		...
		return NewTimestampConvertProcessor(ctx, pipe.FormatName(rt.FormatName(name)))
	})
}
