...
// TestQuery_queryStringLabelMap 测试 queryStringLabelMap 函数的 QueryString 解析功能
func TestQuery_queryStringLabelMap(t *testing.T) {
	testCases := []struct {
		name        string
		queryString string
		expected    map[string][]string
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			query := Query{QueryString: tc.queryString}
			result := query.queryStringLabelMap()
