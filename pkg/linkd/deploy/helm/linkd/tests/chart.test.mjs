...
test("default GHCR release images reach all roles, Console and migration", () => {
  ...
  for (const role of ["control-plane", "cleaner", "lifecycle", "console"]) {
    ...
    assert.equal(byComponent(docs, role).spec.template.spec.containers[0].image, prefix + name + ":0.1.6");
  }
  assert.equal(docs.find(d => d.kind === "Job").spec.template.spec.containers[0].image, prefix + "linkd:0.1.6");
});
