...
func NewSpaceFilter(ctx context.Context, spaceUid string) (*SpaceFilter, error) {
	router := influxdb.GetSpaceTsDbRouter()
	space := router.GetSpace(ctx, spaceUid)
	if space == nil {
		msg := fmt.Sprintf("SpaceID(%s) is not exists", spaceUid)
...
func (s *SpaceFilter) NewTsDBs(
	spaceTable *routerInfluxdb.SpaceResultTable, tableID string, fieldName string, fieldNameExp *regexp.Regexp,
	measurementType string) []*query.TsDBV2 {
...
func (s *SpaceFilter) DataList(tableID TableID, fieldName string, isRegexp bool) ([]*query.TsDBV2, error) {
	...
	if db != "" && measurement != "" {
		...
		_tIDs := s.router.GetDataLabelRelatedRts(s.ctx, db)
		if _tIDs != nil {
			tableIDs = _tIDs
		}
	} else {
	...
	for _, tID := range tableIDs {
		...
		newTsDBs := s.NewTsDBs(spaceRt, tID, fieldName, fieldNameExp, measurementType)
