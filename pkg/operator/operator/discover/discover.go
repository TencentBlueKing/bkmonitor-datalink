...
func (d *BaseDiscover) Stop() {
	...
	logger.Infof("waiting discover [%s] stopped", d.Name())

	d.wg.Wait()
	d.mm.IncStoppedCounter()
	logger.Infof("shutting down discover [%s]", d.Name())
}
...
func (d *BaseDiscover) makeMetricTarget(lbls, origLabels labels.Labels, namespace string) (*target.MetricTarget, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("parse request path error: %v", err)
	}
	metricTarget.Path = requestURL.Path

	params, err := url.ParseQuery(requestURL.RawQuery)
	if err != nil {
		return nil, fmt.Errorf("parse request query error: %v", err)
	}
	...
	if d.BasicAuth != nil && d.BasicAuth.Username.String() != "" && d.BasicAuth.Password.String() != "" {
		...
		if err != nil {
			d.mm.IncGotSecretFailedCounter()
			return nil, fmt.Errorf("failed to get username from secret: %v", err)
		}
		d.mm.IncGotSecretSuccessCounter()
	...
	if d.BearerTokenSecret != nil && d.BearerTokenSecret.Name != "" && d.BearerTokenSecret.Key != "" {
		...
		if err != nil {
			d.mm.IncGotSecretFailedCounter()
			return nil, fmt.Errorf("failed to get bearer token from secret, monitor=%+v, error: %v", d.monitorMeta, err)
		}
		d.mm.IncGotSecretSuccessCounter()
	...
	if d.TLSConfig != nil {
		...
		if d.TLSConfig.CA.Secret != nil {
			...
			if err != nil {
				d.mm.IncGotSecretFailedCounter()
				return nil, fmt.Errorf("failed to get TLS CA from secret, monitor=%+v, error: %v", d.monitorMeta, err)
			}
			d.mm.IncGotSecretSuccessCounter()
		...
		if d.TLSConfig.Cert.Secret != nil {
			...
			if err != nil {
				d.mm.IncGotSecretFailedCounter()
				return nil, fmt.Errorf("failed to get TLS Cert from secret, monitor=%+v, error: %v", d.monitorMeta, err)
			}
			d.mm.IncGotSecretSuccessCounter()
		...
		if d.TLSConfig.KeySecret != nil {
			...
			if err != nil {
				d.mm.IncGotSecretFailedCounter()
				return nil, fmt.Errorf("failed to get TLS Key from secret, monitor=%+v, error: %v", d.monitorMeta, err)
			}
			d.mm.IncGotSecretSuccessCounter()
...
func (d *BaseDiscover) loopHandleTargetGroup() {
	...
	for {
		...
		case <-ticker.C:
			...
			logger.Debugf("discover [%s] got updated at: %v", d.Name(), time.Unix(updatedAt, 0))
...
func (d *BaseDiscover) handleTargetGroup(targetGroup *targetgroup.Group) {
	...
	for _, tlset := range targetGroup.Targets {
		...
		logger.Debugf("discover [%s] populateLabels %+v", d.Name(), res)
		...
		if err != nil {
			d.mm.IncCreatedChildConfigFailedCounter()
			logger.Errorf("failed to marshal target, error: %s", err)
			continue
		}
...
func (d *BaseDiscover) notify(source string, childConfigs []*ChildConfig) {
	...
	for _, cfg := range childConfigs {
		...
		if _, ok := d.childConfigGroups[source][hash]; !ok {
			logger.Infof("discover [%s] adds file on node: %s, filename: %s", d.Name(), cfg.Node, cfg.FileName)
	...
	for _, key := range removed {
		...
		logger.Infof("discover [%s] deletes file on node: %s, filename: %s", d.Name(), cfg.Node, cfg.FileName)
