...
func (s *ScrollTestSuite) TestQueryRawWithScrollBasic() {
	...
	total, list, resultTableOptions, err := queryRawWithScroll(s.ctx, queryTs)

	log.Infof(s.ctx, "Test result - Total: %d, List length: %d, Error: %v", total, len(list), err)
...
func (s *ScrollTestSuite) TestQueryRawWithScrollMultipleQueries() {
	...
	log.Infof(s.ctx, "Multi-query test result - Total: %d, List length: %d, Error: %v", total, len(list), err)
...
func (s *ScrollTestSuite) TestQueryRawWithScrollErrorHandling() {
	...
	log.Infof(s.ctx, "Error handling test result - Total: %d, List length: %d, Error: %v", total, len(list), err)
...
func (s *ScrollTestSuite) TestQueryRawWithScrollSliceParameters() {
	...
	total1, list1, resultTableOptions1, err1 := queryRawWithScroll(s.ctx, queryTs)
	...
	total2, list2, _, err2 := queryRawWithScroll(s.ctx, queryTs)
	...
	log.Infof(s.ctx, "[TEST] Second scroll result - Total: %d, List length: %d", total2, len(list2))
	...
	total3, list3, _, err3 := queryRawWithScroll(s.ctx, queryTs)
	...
	log.Infof(s.ctx, "[TEST] Third scroll result - Total: %d, List length: %d", total3, len(list3))
...
func (s *ScrollTestSuite) TestScrollSliceRotation() {
	...
	for i := 0; i < 6; i++ { // 执行6次查询，应该覆盖所有slice
		...
		total, list, _, err := queryRawWithScroll(s.ctx, queryTs)

		log.Infof(s.ctx, "[TEST] Request %d result - Total: %d, List length: %d, Error: %v",
			requestCount, total, len(list), err)
