...
func setResourceDefinitions(t *testing.T, mr *miniredis.Miniredis, namespace string, defs ...*cmdb.ResourceDefinition) {
	t.Helper()
...
func setRelationDefinitions(t *testing.T, mr *miniredis.Miniredis, namespace string, defs ...*cmdb.RelationDefinition) {
	t.Helper()
	entities := make(map[string]json.RawMessage, len(defs))
	for _, def := range defs {
		b, err := json.Marshal(def)
...
func TestRedisSchemaProvider_LoadResourceDefinitions(t *testing.T) {
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, cmdb.ErrResourceDefinitionNotFound)
	})
}
...
func TestRedisSchemaProvider_LoadRelationDefinitions(t *testing.T) {
	...
	t.Run("NotFound", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, cmdb.ErrRelationDefinitionNotFound)
	})
}
...
func TestRedisSchemaProvider_HotReload(t *testing.T) {
	...
	rd := &cmdb.ResourceDefinition{
		Namespace: "test",
		Name:      "app",
		Fields:    []cmdb.FieldDefinition{{Name: "app_id", Required: true}},
	}
	...
	// 更新 Redis 数据
	rd.Fields = append(rd.Fields, cmdb.FieldDefinition{Name: "version", Required: false})
	setResourceDefinitions(t, mr, "test", rd)
	...
	// 等待热更新完成（轮询，最多 1 秒）
	var updated *cmdb.ResourceDefinition
	for i := 0; i < 20; i++ {
...
func TestRedisSchemaProvider_WithoutReloadOnStart(t *testing.T) {
	...
	// 等待加载完成
	var result *cmdb.ResourceDefinition
	for i := 0; i < 20; i++ {
		result, err = provider.GetResourceDefinition("test", "app")
		if err == nil {
			break
